...
Traps are synchronous events that result from the execution of an instruction. The action of the CPU in response to a trap condition is similar to the case of an interrupt in interrupt mode 3 (see [Chapter 6](6-Interrupts_And_Traps.md)). All traps except for Extended Instruction, System Stack Overflow Warning, Single Step and Breakpoint-on-Halt are nonmaskable.
...
This section presents an overview of the Z280 instruction set, arranged by functional groups. (See [Section 5.5](5-Instruction_Set.md#55-notation-and-binary-encoding) for an explanation of the notation used in Tables 5-2 through 5-11.)
...
The interrupt mode of operation is set to one of four modes (see [Chapter 6](6-Interrupts_And_Traps.md) for a description of the various modes for responding to interrupts). The current interrupt mode can be read from the Interrupt Status register.
...
A special sequence of bus transactions is performed when this instruction is encountered in order to control Z80 family peripherals; see [Chapter 12](12-Z280_Bus_External_Interface.md).
